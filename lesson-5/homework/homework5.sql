...
select productname, price, iif(price >1000, 'expensive', 'Affordable') from products

---- 11 Write a query to find employees in the Employees table who have either Age < 25 or Salary > 60000.

select firstname , age , salary from Employees where age <25 or salary > 60000

-- 12  Update the salary of an employee based on their department, increase by 10% if they work in 'HR' or EmployeeID = 5
 
 select firstname , age , salary, (salary +(salary * 0.10)) as updated_salary from Employees where DepartmentName in ('HR') or EmployeeID = 5
...
*/
--- 13 Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' if SaleAmount BETWEEN 200 AND 500, 
